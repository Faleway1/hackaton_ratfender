...
class Tower {
  ...
  initRangeVisual() {
    ...
    this.rangeGraphic.circle(
      0,
      0,
      (this.stats.range + 0.5) * GAME_SETTINGS.TILE_WIDTH
    );
  ...
  render(x, y, placeIt) {
    ...
    const cell = gridManager.findOnGrid(x, y, game.cellsList);
    const middleOfTile = {
    ...
    if (!this.enemies_in_range.length) {
      ...
      for (let i = 0; i < this.stats.nb_shots; i++) {
        ...
        if (this.canAttackEnemy(this.enemies_in_range[i]) === true) {
          this.enemies_in_range[i].takeDamage(
            this.stats.attack * this.multiplier
          );
    ...
    path1.addEventListener("click", () => {
      if (this.level.path1 === 3) {
        ...
        if (this.upgradePrice.path1Upgradecost <= game.pdr) {
          ...
          this.upgradePrice.path1Upgradecost =
            this.upgradePrice.path1Upgradecost + 20 * this.level.path1;
    ...
    path2.addEventListener("click", () => {
      if (this.level.path2 === 3) {
        ...
        if (this.upgradePrice.path2Upgradecost <= game.pdr) {
          ...
          game.money.textContent = game.pdr;
          this.upgradePrice.path2Upgradecost =
            this.upgradePrice.path2Upgradecost + 20 * this.level.path2;
    ...
    path3.addEventListener("click", () => {
      if (this.level.path3 === 3) {
        ...
        if (this.upgradePrice.path3Upgradecost <= game.pdr) {
          ...
          game.money.textContent = game.pdr;
          this.upgradePrice.path3Upgradecost =
            this.upgradePrice.path3Upgradecost + 20 * this.level.path3;
